...
RSpec.describe PDK::Util::Bundler do
  ...
  describe PDK::Util::Bundler::BundleHelper do
    ...
    describe '#installed?' do
      ...
      context 'all gems are present' do
        it 'returns true' do
          expect(instance.installed?).to be true
        end
      end

      context 'has missing gems' do
        let(:gemfile) { File.join(FIXTURES_DIR, 'module_gemfile') }
      ...
      context 'with gem overrides' do
        ...
        it 'updates env before checking bundler specs' do
          allow(PDK::Util::RubyVersion).to receive(:active_ruby_version).and_return('2.5.3')
          expect(ENV).to receive(:[]=).with('PUPPET_GEM_VERSION', '1.2.3')
          expect(ENV).to receive(:[]=).with('BUNDLER_SPEC_RUBY_VERSION', '2.5.3')
          expect(ENV).to receive(:[]=).with('BUNDLER_SPEC_RUBY_VERSION', nil)
