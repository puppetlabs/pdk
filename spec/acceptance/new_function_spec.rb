    ...
    context 'in a new module' do
      ...
      context 'when creating the function' do
        describe command('pdk new function -t v4 abs') do
          ...
          its(:stdout) { is_expected.to match(/#{File.join('lib', 'puppet', 'functions', 'abs.rb')}/) }
          its(:stdout) { is_expected.to match(/#{File.join('dspec', 'functions', 'abs_spec.rb')}/) }
        ...
        describe command('pdk new function -t native abs') do
          its(:exit_status) { is_expected.to eq 0 }
          its(:stdout) { is_expected.to match(/Files added/) }
          its(:stdout) { is_expected.to match(/#{File.join('functions', 'abs.pp')}/) }
          its(:stdout) { is_expected.to match(/#{File.join('spec', 'functions', 'abs_spec.rb')}/) }
          its(:stderr) { is_expected.to have_no_output }
