  ...
  context 'in a new module' do
    ...
    context 'when creating a test for the main class' do
      describe command('pdk new test --unit new_unit_test') do
        ...
        its(:stdout) { is_expected.to match(/#{File.join('spec', 'classes', 'new_unit_test_spec.rb')}/) }
        its(:stderr) { is_expected.to match(/Using Ruby/) }
        its(:stderr) { is_expected.to match(/Using Puppet/) }

        describe file(File.join('spec', 'classes', 'new_unit_test_spec.rb')) do
          it { is_expected.to be_file }

          its(:content) do
            is_expected.to match(/describe 'new_unit_test' do/)
          end
        end
      end
    end
    ...
    context 'when creating a test for a defined type' do
      describe command('pdk new test def_type') do
        its(:exit_status) { is_expected.to eq 0 }
        its(:stdout) { is_expected.to match(/#{File.join('spec', 'defines', 'def_type_spec.rb')}/) }
        its(:stderr) { is_expected.to match(/Using Ruby/) }
        its(:stderr) { is_expected.to match(/Using Puppet/) }
        ...
        describe file(File.join('spec', 'defines', 'def_type_spec.rb')) do
          ...
          its(:content) do
            is_expected.to match(/describe 'new_unit_test::def_type' do/)
          end
        end
      end
    end
