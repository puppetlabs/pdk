...
describe 'pdk new provider', :module_command do
  ...
  context 'when run inside of a module' do
    ...
    describe command('pdk new provider test_provider') do
      its(:stdout) { is_expected.to match(/#{File.join('lib', 'puppet', 'type', 'test_provider.rb')}/) }
      its(:stdout) { is_expected.to match(/#{File.join('lib', 'puppet', 'provider', 'test_provider', 'test_provider.rb')}/) }
      its(:stdout) { is_expected.to match(/#{File.join('spec', 'unit', 'puppet', 'provider', 'test_provider', 'test_provider_spec.rb')}/) }
      its(:stdout) { is_expected.to match(/#{File.join('spec', 'unit', 'puppet', 'type', 'test_provider_spec.rb')}/) }
      ...
      describe file(File.join('lib', 'puppet', 'type', 'test_provider.rb')) do
        ...
        its(:content) { is_expected.to match(/Puppet::ResourceApi.register_type/) }
        its(:content) { is_expected.to match(/name: 'test_provider'/) }
      end
      ...
      describe file(File.join('lib', 'puppet', 'provider', 'test_provider', 'test_provider.rb')) do
        it { is_expected.to be_file }
        its(:content) { is_expected.to match(/class Puppet::Provider::TestProvider::TestProvider/) }
      end
      ...
      describe file(File.join('spec', 'unit', 'puppet', 'provider', 'test_provider', 'test_provider_spec.rb')) do
        it { is_expected.to be_file }
        its(:content) { is_expected.to match(/RSpec.describe Puppet::Provider::TestProvider::TestProvider do/) }
      end
      ...
      describe file(File.join('spec', 'unit', 'puppet', 'type', 'test_provider_spec.rb')) do
        it { is_expected.to be_file }
        its(:content) { is_expected.to match(/RSpec.describe 'the test_provider type' do/) }
      end
