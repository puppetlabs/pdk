      ...
      class BundleHelper
        ...
        def gemfile_definition(gem_overrides = {})
          ...
          @gemfile_definition ||= begin
            ...
            env_vars['BUNDLER_SPEC_RUBY_VERSION'] = PDK::Util::RubyVersion.active_ruby_version
            env_vars.each { |name, value| ENV[name.to_s] = value }
            # set the desired ruby version so the Gemfile renders correctly
            ::Bundler.configure
            ::Bundler::Definition.build(gemfile, gemfile_lock, nil)
          ensure
            # reset env variable back to nil so we don't cause environment pollution
        ...
        def installed?(gem_overrides = {})
          ...
          # true if missing_specs is empty
