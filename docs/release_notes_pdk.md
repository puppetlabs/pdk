...
Fix inconsistencies between the `pdk validate` and `pdk release prep` commands. The `in_module_root` function now checks to see if `metadata.json` is present instead of relying on folder names [PDK-1758](https://tickets.puppetlabs.com/browse/PDK-1758)
